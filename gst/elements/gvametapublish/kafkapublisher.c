...
void kafka_open_connection(KafkaPublishConfig *publishConfig, rd_kafka_t *producerHandler, rd_kafka_topic_t *rkt) {
    ...
    rkt = rd_kafka_topic_new(producerHandler, publishConfig->topic, NULL);
    if (!rkt) {
...
void kafka_close_connection(rd_kafka_t *producerHandler, rd_kafka_topic_t *rkt) {
